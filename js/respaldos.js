...
async function obtenerDatosRespaldo(){
  const datos={
    meta:{
      app:'Quiniela Copa MX',
      version:'MODULAR_RESPALDOS',
...
async function guardarRespaldo(tipo,automatico=false){
  ...
  // Auditoría omitida en respaldos para evitar conflictos 400/409.
  // El respaldo sí queda guardado correctamente en la tabla respaldos.
...
async function revisarRespaldoAutomatico(){
  try{
    ...
    const r=await db
      .from('respaldos')
      .select('*')
      .eq('tipo','AUTO_SEMANAL')
      .maybeSingle();

    if(r.error){
      console.warn('Respaldos no disponibles:',r.error.message);
      return;
    }
    ...
    console.warn('Respaldo automático:',e.message);
  }
}
async function cargarEstadoRespaldos(){
  ...
  try{
    ...
    if(box){
      ...
      box.innerHTML=
        `Último automático: <b>${auto?fecha(auto.fecha)+' · '+esc(auto.semana||''):'No existe'}</b><br>`+
        `Último manual: <b>${man?fecha(man.fecha)+' · '+esc(man.semana||''):'No existe'}</b>`;
    }

  }catch(e){
    if(box){
      box.innerHTML='Falta correr el SQL de respaldos o revisar permisos: '+esc(e.message);
    }
  }
}
async function crearRespaldoManual(){
  try{
    ...
    msg('Error respaldo: '+e.message,'error');
  }
}
async function forzarRespaldoAutomatico(){
  try{
    ...
    msg('Error respaldo automático: '+e.message,'error');
  }
}
...
async function descargarRespaldo(tipo){
  try{
    ...
    msg('Error al descargar respaldo: '+e.message,'error');
  }
}
async function restaurarRespaldo(tipo){
  try{
    if(!esAdmin) return msg('Solo administrador','error');
    const r=await obtenerRespaldoGuardado(tipo);
    if(!confirm(`Restaurar respaldo ${tipo} del ${fecha(r.fecha)}? Esto reemplazará jornadas, boletos, partidos, pronósticos, pagos, fama, auditoría y tabla general. Equipos y escudos se conservan.`)){
      return;
    }
    const txt=prompt('Para confirmar escribe: RESTAURAR RESPALDO');
    if(txt!=='RESTAURAR RESPALDO'){
      return msg('Restauración cancelada','info');
    }
    ...
    for(const t of orden){
      ...
      if(arr.length){
        ...
        if(ins.error){
          throw new Error(`Error restaurando ${t}: ${ins.error.message}`);
        }
      }
    }
    jornadaVistaId=null;
    ...
    msg('Error al restaurar: '+e.message,'error');
  }
}
